...
const Vinilos = (() => {
  // i18n: TR(clave, params) traducido vía I18n (capa opcional). Sin I18n → devuelve la clave.
  // (se llama TR y no T porque create() usa una coordenada local `T` = borde superior).
  const TR = (k, p) => (typeof I18n !== 'undefined' && I18n.t) ? I18n.t(k, p) : k;
  const eraName = k => TR('vin.era.' + k);   // nombre de época traducido (k = clave interna fija)
  const ERAS = [
  ...
  function create(ctx) {
    ...
    function interact() {
      const b = nearBin();
      if (b) { const e = ERAS[b.era]; setMsg(TR('vin.dig', { era: eraName(e.k), disc: e.disc[(Math.random()*e.disc.length)|0] })); return; }
      if (nearPunk()) {
        if (P.coins < 8) { setMsg(TR('vin.punk.noFunds')); return; }
        P.coins -= 8;
        if (!P.hasCementoTicket) {
          P.hasCementoTicket = true;
          setMsg(TR('vin.punk.ticket'), 9);
        } else setMsg(TR('vin.punk.again'));
        return;
      }
      setMsg(TR('vin.hint'));
    }
    ...
    return {
      ...
      update(dt) {
        ...
        const b = nearBin();
        prompt = b ? TR('vin.prompt.gondola', { era: eraName(ERAS[b.era].k) }) : nearPunk() ? TR('vin.prompt.punk') : (Math.abs(player.x-exit.x)<40 && player.y>B-50 ? TR('vin.prompt.exit') : '');
      },
      draw(ctx, VW, VH) {
        ...
        // afiches en la pared
        ctx.fillStyle = '#7CFC00'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left'; ctx.fillText(TR('vin.banner'), L+8, T+16);
        ...
        for (const b of bins) {
          ...
          ctx.fillStyle = e.c; ctx.font = 'bold 13px monospace'; ctx.textAlign = 'center'; ctx.fillText(eraName(e.k), b.cx, b.cy+b.h/2+14);
        }
        // mostrador + punk
        ...
        // salida
        ctx.fillStyle = '#2e7d32'; ctx.fillRect(exit.x-22, B-10, 44, 14); ctx.fillStyle = '#eaffea'; ctx.font = 'bold 9px monospace'; ctx.fillText(TR('vin.exitLabel'), exit.x, B);
        // jugador
        ...
        ctx.fillStyle = '#e0b0ff'; ctx.font = 'bold 13px monospace'; ctx.textAlign = 'left'; ctx.fillText(TR('vin.title'), 10, 18);
