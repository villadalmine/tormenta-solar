...
- **Estado:** **F1-F3+fog (v289) + FIXES del playtest (v293):** quests ancladas a UN nodo (la entrada del edificio; sin spam de 🔒 — las futuras solo en tooltip), **CLICK = zoom al edificio** (`Mapa.hitTest`; Esc sale del zoom primero), columnas reservadas para sub-modos (sin superposición), zoom con etiqueta de piso (P7/S2) + marcadores grandes + fila resaltada si hay contenido. **v294:** anclaje FINO por campo `sala` de las aristas (fichas ```hist → "piso 19"/"búnker"): en el zoom cada quest va a SU piso; arista `tesoro` nueva (19/19 ancladas, acentos normalizados, sub-modos sin sala → calle). **v295-296:** quests NOMBRADAS en el zoom (⭐ primero, +N) + grafo bilingüe (`title_en` en las 19 aristas) + fog SUAVE (nombres siempre, '???' muere; secretas ocultas) + quests ⭐ visibles sin visitar (guían) + puertas de la calle sin sala (el súper) derivadas del DATO + zig-zag/presupuesto de etiquetas anti-solape. **v298:** DOS VISTAS con pestañas ([1] MANZANA solo superficie / [2] SUBSUELOS grande con S1-S4 y hermanas repartidas), huérfanas sin puerta de entrada (cuevas del cuevero, spawnIn) adoptadas por su sala de salida, barras RECORTADAS adyacentes (no pisadas). **v299 (REDISEÑO por capas, feedback dueño):** vista general = CAJONES por edificio (`model.buildings` derivado de los grupos: ×pisos, 🔦descubierto v/N, ✅n ⭐n, ?? si hay quest bloqueada) en flujo sin solapes; click → PISOS del edificio; [2] SUBSUELOS. Foco: qué descubriste / qué hitos / dónde hay algo. **v300:** orientación espacial (columna del cajón = x real en la calle, colisión apila; ticks ▾ por puerta; línea al hover) + columna 🎮 MULTIJUGADOR con info real (piquete ✊n/5, bodegón 1v1·6, 👥 online por espacio) + textos con presupuesto en px + [Z] coherente + el bot valida cajones sin solapes y captura las vistas (autoplay 07). **v301:** 3ª vista **LA CUADRA** (skyline en perspectiva: siluetas x real + altura=pisos, cara lateral, ⭐/?? en azotea, obelisco de fondo, bodegón/telo en la azotea del cine, hover=etiqueta flotante, click=detalle; `Mapa.sky` exportado, bot la valida). Pestañas [1] cuadra [2] manzana [3] subsuelos. **v302-303:** skyline con solver 1D anti-solape (check pairwise en el bot) + bolsillos secretos por teleport fuera del mapa (telohab) + **Ayuda del mapa en ⚙** (`ts_ayuda_facil`): DIFÍCIL = ?? misterioso · FÁCIL = todo marcado con nombre (🔒 en tooltips y pisos). Falta: cursor por teclado, minimapa HUD, `map` component override, online POR sala.
