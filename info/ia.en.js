...
(async () => {
  ...
  try {
    ...
    $('cadena').innerHTML = '<b>Which model each part of the game uses, RIGHT NOW</b> <span class="muted">(in parentheses, the real model behind the alias)</span>:' +
      ...
      '<div class="muted" style="margin:.2rem 0 .45rem 1.5rem">🧠 <b>LIVE now (premium, v373):</b> individual memory — chat-capable characters (the priest, the soup-kitchen leader, the card shark, the oracles…) remember specific things ABOUT YOU, not just the generic neighborhood-wide memory everyone sees. Added NPC by NPC by editing their graph entry.</div>' +
